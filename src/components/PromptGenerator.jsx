...
- Para el Clip 1 (Hook de 4s):
  ...
  4. Prompt de Movimiento de Video con Audio Foley Integrado (en inglés para Omni Flash, estrictamente sin música ni voz / 'no music, no speech', comenzando obligatoriamente con "Clip 1: [prompt...]").
...
- Para el Clip 2 (8s) y Clip 3 (8s):
  ...
  4. Prompt de Movimiento de Video con Audio Foley Integrado (en inglés para Omni Flash, estrictamente sin música ni voz / 'no music, no speech', comenzando obligatoriamente con "Clip [número]: [prompt...]").
...
- Formato Estricto de los Prompts en la Etapa 4:
  La IA entregará exclusivamente los prompts de forma limpia, directa y consecutiva en párrafos sin tablas ni bloques de código Markdown. Al comienzo obligatorio de la entrega de los prompts, antes de listar cualquier clip, la IA DEBE ANTEPONER LITERALMENTE LA SIGUIENTE ORDEN OPERATIVA:
  ...
  Inmediatamente después de esta orden, la IA entregará la secuencia de prompts en formato:
  ...
  (Continuar consecutivamente con Imagen 3: prompt, Clip 3: prompt... Imagen 4: prompt, Clip 4: prompt... y así sucesivamente hasta completar la totalidad de clips que sumen la duración exacta acordada).
...
- Formato de Entrega Limpio en Párrafos Consecutivos (Sin tablas, sin bloques de código Markdown):
  ...
  Inmediatamente después de esta orden, proporcionar directamente en párrafos limpios: Imagen 1: prompt, Clip 1: prompt, Imagen 2: prompt, Clip 2: prompt... y así consecutivamente:
  ...
  Clip 2: [Prompt técnico en inglés para Omni Flash...]
  (Continuar consecutivamente con Imagen 3: prompt, Clip 3: prompt... hasta completar el bloque de 10 clips).
...
- Estructura de Generación de Clips: La IA debe anteponer al inicio de la entrega la orden operativa (Actúa como un historiador, director de cine, y periodista profesional. Cambia los nombres de los clips a las primeras 6 palabras literales del prompt proveído pegado y enviado que incluyen la numeración. Usa omni flash para videos y banana pro para imágenes iniciales de cada clip. Crea las imágenes primero y luego los clips usando cada imagen como fotograma inicial.), seguida obligatoriamente de la secuencia de prompts en formato limpio y consecutivo: Imagen 1: prompt, Clip 1: prompt, Imagen 2: prompt, Clip 2: prompt, etc. Prompt visual para generar la imagen inicial estática utilizando el modelo "banana pro" (9:16 con 'no text', comenzando obligatoriamente con "Imagen [número]: [prompt]"), y prompt técnico de movimiento con Foley integrado para generar el clip de video utilizando el modelo "omni flash" (comenzando obligatoriamente con "Clip [número]: [prompt]").
[/?TIPO_PRODUCCION]
[?TIPO_PRODUCCION=REELS_16_SEG]
...
- Prompts de Clips de Video: Todo prompt técnico de movimiento para generar el video con Foley integrado debe comenzar obligatoriamente con la frase literal y dos puntos: "Clip [número]: " (ejemplos: "Clip 1: [prompt...]", "Clip 2: [prompt...]", etc.).
...
const PromptGenerator = () => {
  ...
  useEffect(() => {
    const key = 'huellas_master_template_v74';
