...
Una vez confirmado el tema por el usuario, la IA redactará EXCLUSIVAMENTE el guion completo de la voz narrativa (voiceover) para el video vertical de 1 minuto (~110-130 palabras, calculado para un ritmo solemne y fluido de 112-114 WPM):
- Hook de Inicio Obligatorio (Lugar, Fecha e Intriga): El preámbulo al principio del guion tiene que generar un hook demoledor; los primeros segundos tienen que capturar al consumidor con algo profundamente intrigante o terrorífico. La narración DEBE EMPEZAR OBLIGATORIAMENTE POR EL LUGAR Y LA FECHA (ej: "Londres, 1888...", "París, invierno de 1789...", "En las profundidades del Mar del Norte, 1943..."), seguido inmediatamente por el suceso perturbador, el misterio o la revelación que detenga el scroll al instante.
...
3. Prompt de Imagen Inicial: En idioma inglés optimizado para "banana pro" (formato vertical 9:16, con 'no text', cerrando obligatoriamente con la barrera sonora y sufijos).
4. Prompt de Movimiento de Video con Audio Foley Integrado: En idioma inglés optimizado para el modelo de video "omni flash", integrando dentro del propio prompt la descripción sonora inmersiva (ambient sound, foley acústico y texturas físicas, sin música ni voz).
...
- Voz en Off y Foley: Acompañado de locución narrativa solemne y continua durante todo el minuto (estrictamente sin silencios estructurales ni pausas artificiales /2s o /#s), complementada con el paisaje sonoro de Foley integrado en cada prompt de video. El preámbulo inicial de la voz en off debe comenzar obligatoriamente por el LUGAR y la FECHA, capturando de inmediato en los primeros segundos con un gancho profundamente intrigante o terrorífico. La voz en off debe narrar los hechos en tercera persona de forma directa y cinematográfica, sin hablar jamás en primera persona del plural ("nosotros"). Además, no es necesario decir "x tipo de persona o profesión anónimos" para forzar anonimización; las figuras históricas y colectivos se nombran con total naturalidad según la narrativa.
- Estructura de Generación de Clips (8 Clips): La IA debe entregar obligatoriamente la estructura de los 8 clips conteniendo: 1) Identificación del clip y selección de cámara, 2) Explicación visual del clip en español, 3) Prompt visual para generar la imagen inicial estática utilizando el modelo "banana pro" (9:16 con 'no text'), y 4) Prompt técnico de movimiento con Foley integrado para generar el clip de video a partir de dicha imagen utilizando el modelo de video "omni flash".
[/?TIPO_PRODUCCION]
[?TIPO_PRODUCCION=REELS_16_SEG]
...
const PromptGenerator = () => {
  ...
  useEffect(() => {
    const key = 'huellas_master_template_v56';
