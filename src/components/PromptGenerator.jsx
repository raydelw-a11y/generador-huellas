...
-> ALTO INQUEBRANTABLE: La IA entrega exclusivamente los prompts de imágenes modelo de referencia precedidos por la orden en una sola oración ("Cambia los nombres y ponles @ seguido del nombre de la persona correspondiente."), y concluye su respuesta deteniéndose. Espera la confirmación del usuario antes de pasar a la selección del texto en pantalla del Clip 1 (Etapa 5).

ETAPA 5: SELECCIÓN DEL TEXTO EN PANTALLA DEL CLIP 1 (PRIMEROS 6 SEGUNDOS DEL VIDEO)
Tras la validación de las imágenes modelo de referencia de la Etapa 4 y ANTES de pasar a los prompts de imágenes iniciales y clips de video, la IA DEBE ACORDAR OBLIGATORIAMENTE CON EL USUARIO LA SELECCIÓN DEL TEXTO EN PANTALLA DEL CLIP 1 (correspondiente a los primeros 6 segundos del video):
[?TEXTO_PANTALLA_CLIP_1=CAPCUT_MANUAL]
...
-> ALTO INQUEBRANTABLE: La IA debe detenerse aquí y esperar la respuesta y confirmación del usuario con el texto elegido para el Clip 1 antes de pasar a la siguiente etapa (Etapa 6: Prompts de Imágenes Iniciales y Clips).
[/?TEXTO_PANTALLA_CLIP_1]
[?TEXTO_PANTALLA_CLIP_1=IA_CLIP_1]
...
-> ALTO INQUEBRANTABLE: La IA formula la propuesta de opciones de texto en pantalla para el Clip 1 y concluye su respuesta deteniéndose. Espera la selección y confirmación del usuario antes de pasar a la siguiente etapa (Etapa 6: Prompts de Imágenes Iniciales y Clips).
[/?TEXTO_PANTALLA_CLIP_1]

ETAPA 6: PROMPTS DE IMÁGENES INICIALES Y CLIPS (CONSULTA DE ENTREGA: DE 10 EN 10 O TODOS JUNTOS)
Tras la selección y confirmación del texto en pantalla del Clip 1 (Etapa 5), y ANTES de generar o entregar cualquier prompt de imagen inicial o clip de video, la IA DEBE CONSULTAR OBLIGATORIAMENTE AL USUARIO cómo desea recibir los prompts:
...
- Protocolo Obligatorio de Consulta y Entrega de Prompts (De 10 en 10 o Todos Juntos):
  ...
  * Modalidad 1: Entrega de 10 en 10:
    ...
    - Si la producción tiene 10 clips o menos en total (ej. un video de 60s con 7 u 8 clips), la IA entregará la totalidad de los clips en un único bloque y se detendrá con el ALTO INQUEBRANTABLE antes de pasar a la siguiente etapa (Etapa 7: Metadata para Subir el Video).
  * Modalidad 2: Entrega de Todos Juntos:
    ...
    - Al finalizar la entrega completa de todos los clips, la IA se detendrá con el ALTO INQUEBRANTABLE antes de pasar a la siguiente etapa (Etapa 7: Metadata para Subir el Video).
...
[/?SINCRONIZACION_CLIP_1]
[/?VOZ_NARRATIVA_EN_CLIPS]
-> ALTO INQUEBRANTABLE: La IA entrega el bloque de 10 clips y concluye su respuesta deteniéndose. Espera la confirmación del usuario para entregar el siguiente bloque (Clips 11 al 13 o restantes, el cual iniciará directamente con Image 11: Create an image [prompt], Clip 11 (01:36 - 01:46): Create a video [prompt], sin la orden operativa). Una vez completados y entregados todos los clips de video del proyecto, la IA pasará obligatoriamente a la Etapa 7 (Metadata para Subir el Video). NO generes la metadata de publicación en esta etapa.

ETAPA 7: METADATA PARA SUBIR EL VIDEO (ENTREGA POSTERIOR)
Una vez completada y confirmada la entrega de todos los clips de video de la Etapa 6, en una interacción posterior la IA generará la metadata de publicación en formato "Todo Corrido" y SIN encabezados de sección (cero "Título:", cero "Descripción:", cero "Comentario Fijado:"):
...
[/?TEXTO_PANTALLA_CLIP_1]
[?TEXTO_PANTALLA_CLIP_1=CAPCUT_MANUAL]
Texto en Pantalla del Clip 1: El texto en pantalla del primer clip lo insertaré en edición manual en CapCut (los prompts de Banana Pro y Omni Flash llevan estrictamente 'no text'; la IA proporciona las opciones de texto en la Etapa 5 para montaje manual).
[/?TEXTO_PANTALLA_CLIP_1]
[?DURACION_CLIPS=10_SEGUNDOS]
...
- Orden Operativa en una Sola Oración para Prompts de Referencia (Etapa 4): Al inicio obligatorio de la entrega de los prompts de imágenes modelo de referencia de personas, lugares y objetos, la IA debe colocar obligatoriamente como preámbulo inicial en una sola oración la siguiente instrucción textual:
  Cambia los nombres y ponles @ seguido del nombre de la persona correspondiente.
- Regla Estricta: Los '@' en las Imágenes de Referencia son Solo de Personas y Máximo un Solo '@' por Prompt (Etapa 4 y Etapa 6): Los '@' en las imágenes de referencia son ÚNICA Y EXCLUSIVAMENTE DE PERSONAS. Queda terminantemente prohibido colocar el símbolo '@' en nombres de lugares u objetos en las imágenes de referencia (los lugares y objetos se crean y nombran estrictamente sin '@'). Asimismo, tanto en los prompts de imágenes modelo de referencia de personas (Etapa 4) como en los prompts de imágenes iniciales y clips de video (Etapa 6), SOLO HABRÁ COMO MÁXIMO UN ÚNICO SÍMBOLO '@' POR CADA PROMPT, correspondiente exclusivamente al personaje principal (ej. '@Jack The Ripper in Whitechapel, London, England'). Si una escena o clip no incluye personajes con '@', el lugar y tiempo se redactan limpiamente sin ningún '@' (ej. 'in Whitechapel, London, England, 1888...'). En los prompts de imágenes y clips, solo habrá un '@' por cada uno y será únicamente de personas.
- Regla Inquebrantable: Imágenes de Referencia Exclusivas para Referencia Visual (Prohibido Usarlas como Primer Fotograma del Video): Las imágenes generadas en la Etapa 4 (modelos de personas con '@' [@Personas] y modelos de lugares y objetos sin '@') son ÚNICA Y EXCLUSIVAMENTE para referencia visual en la herramienta de IA (actúan como activos de referencia / asset references para mantener consistencia fisonómica, arquitectónica y de objetos). Queda TERMINANTEMENTE PROHIBIDO utilizar estas imágenes de referencia como primer fotograma (fotograma inicial) de ningún clip de video. El primer fotograma de cada clip de video se genera obligatoria y exclusivamente en la Etapa 6 con su prompt numerado correspondiente (Image 1 para Clip 1, Image 2 para Clip 2, etc.).
- Orden Operativa Exclusiva al Inicio del Primer Envío de Prompts: Al inicio obligatorio del primer envío de prompts de imágenes y videos (Bloque 1 si se eligió de 10 en 10, o al inicio de la entrega si se eligió todos juntos), la IA debe colocar obligatoriamente como preámbulo inicial la siguiente orden operativa textual (quedando estrictamente prohibido repetirla en bloques posteriores si se entrega de 10 en 10):
  ...
  rename everything adding _ as the blank spaces between words
  from 1 to 9 change them to 01 to 09
  (Aclaración Fundamental: Las imágenes de referencia creadas previamente en la Etapa 4 [las personas creadas con '@' y los lugares u objetos sin '@'] son única y exclusivamente para referencia visual y NO para ser usadas como primer fotograma del video. La orden de usar "cada imagen como fotograma inicial" aplica estricta y únicamente a las imágenes iniciales numeradas de esta etapa: Image 1 para Clip 1, Image 2 para Clip 2, etc.).
- Prompts de Imagen Inicial con "Create an image", Tiempo, Lugar y Regla de un Solo '@' Exclusivo para Personas: Todo prompt para generar la imagen fija inicial debe estar redactado al 100% en idioma inglés, comenzando obligatoriamente tras la numeración con la frase: "Image [número]: Create an image " (o "Imagen [número]: Create an image "), e incluir obligatoriamente el TIEMPO y LUGAR como anclaje histórico visual. Si aparece un personaje principal modelado en la Etapa 4, se incluye con '@' (ej. '@Jack The Ripper in Whitechapel, London, England, 1888...'), y si no hay personaje, el lugar y tiempo van sin '@' (ej. 'in Whitechapel, London, England, 1888...'), manteniendo estrictamente que los '@' sean solo de personas y como máximo un único '@' en todo el prompt.
[?VOZ_NARRATIVA_EN_CLIPS=INCLUIR]
...
[?TEXTO_PANTALLA_CLIP_1=CAPCUT_MANUAL]
- Regla de Texto en Pantalla en Prompts (Edición Manual en CapCut): Todos los prompts de imagen y video se mantienen estrictamente con la directriz 'no text'. El texto de apertura se selecciona previamente con el usuario en la Etapa 5 y se coloca manualmente en edición en CapCut durante el montaje.
[/?TEXTO_PANTALLA_CLIP_1]
[?TEXTO_PANTALLA_CLIP_1=IA_CLIP_1]
- Regla de Texto en Pantalla en Prompts (Generado por IA en Clip 1): El texto en pantalla para la apertura se selecciona previamente con el usuario en la Etapa 5 y va ÚNICA Y EXCLUSIVAMENTE en el prompt de video del Clip 1 de [DURACION_CLIP_1] segundos, generado directamente por la IA tipo comienzo de un documental cinematográfico (Ciudad, País, Tiempo en la primera línea y justo debajo el Gancho intrigante con tipografía cinematográfica integrada). Queda TERMINANTEMENTE PROHIBIDO incluir el texto en el prompt de la Imagen 1 (la Imagen 1 lleva estrictamente 'no text'). Asimismo, la IA debe incluir la indicación operativa: "En edición, bajar la velocidad del Clip 1 de [DURACION_CLIP_1]s para llevarlo a [DURACION_CLIP_1_RALENTIZADO] segundos de duración". Los prompts de video restantes van limpios sin texto en pantalla.
[/?TEXTO_PANTALLA_CLIP_1]
...
const PromptGenerator = () => {
  ...
  useEffect(() => {
    const key = 'huellas_master_template_v129';
